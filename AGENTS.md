...
- `demos/`: live demo playbooks for slide 7. `01-authoring/` (agent writes a test) and `02-debug-loop/` (agent fixes a failing test using `npx playwright trace` from 1.59). Each has a README with prompt, expected behavior, narration notes, and fallback.
