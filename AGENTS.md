...
- `demos/`: live demo playbook for slide 7. `01-authoring/` shows agent writing a test without MCP (inferred locators from source) and with Playwright MCP (real locators from accessibility snapshot). Has prompts, expected behavior, narration notes, and fallback.
